...
public:
	...
	RedBlackNode<T> operator=(const RedBlackNode<T>& rbn);
	bool operator!=(RedBlackNode<T> rbn);
	bool operator==(RedBlackNode<T> rbn);
	bool operator<=(RedBlackNode<T> rbn);
	bool operator<(RedBlackNode<T> rbn);
	bool operator>=(RedBlackNode<T> rbn);
...
template<class T>
RedBlackNode<T> RedBlackNode<T>::operator=(const RedBlackNode<T>& rbn)
{
	_v = rbn._v;
	parent = rbn.parent;
	left = rbn.left;
...
template<class T>
bool RedBlackNode<T>::operator!=(RedBlackNode<T> rbn)
{
	if (_v != rbn) return true;
	else return false;
}

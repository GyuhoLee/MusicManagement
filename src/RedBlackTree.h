...
protected:
	...
	// v값을 가지는 노드를 찾아서 리턴하고, v값에 받아온다.
	RBNode * _search_and_get(T& v);

	// n으로 부터 LEFT MOST INTERNAL 노드를 찾아서 리턴한다.
	RBNode *_search_lmi(RBNode *n);

	// 노드 n을 삭제한다.
	bool _erase(RBNode *n);
...
public:
	...
	*/
	...
	/**
	*	@brief	트리에 새로운 아이템을 추가한다.
	*	@pre	트리가 비었다면, 초기화가 되어있어야 한다.
	*	@pram	새로 추가할 data
	*	@post	트리에 새로운 아이템이 추가된다.
	*/
	int insert(T value);

	/**
	*	@brief	트리에서 해당 아이템을 제거한다.
	*	@pre	트리가 비었다면, 초기화가 되어있어야 한다.
	*	@post	트리에서 해당 아이템이 제거된다.
	*	@pram	삭제할 data
	*	@return	해당 아이템이 존재해 삭제 성공시 true 반환
	*/
	bool erase(T &value);
	...
	*/
	...
	/**
	*	@brief	트리가 비었는지 반환하는 함수
	*	@pre	트리가 비었다면, 초기화가 되어있어야 한다.
	*	@return	트리가 비었다면 true, 안비었다면 false
	*/
...
template <class T>
RedBlackNode<T> *RedBlackTree<T>::_search_and_get(T& v)
{
	RBNode	*cur = root();
...
// 노드 n을 삭제한다.
template <class T>
bool RedBlackTree<T>::_erase(RBNode *n)
{
	RBNode *pn, *lmi;
	T tmp;
	RBNode *rn;
...
{
	return _erase(_search_and_get(v));
}
...
{
	if (root_ == nullptr)	//root가 비었다(=트리가 비었다)
		return true;	//참을 반환
	return false;	//안비었다면 거짓을 반환
}
...
template<class T>
int RedBlackTree<T>::GetLength() const
{
	return length_;	//개수를 반환함.
}

template<class T>
int CountNodes(RedBlackNode<T> *root)
{
	//GeneralCase:현재 노드의 자식 노드가 존재한다.
	//BaseCase:현재 노드가 nullptr
	if (root == nullptr)	//BaseCase
		return 0;
	else	//GeneralCase
		return CountNodes(root->left()) + CountNodes(root->right()) + 1;	//좌+우+1 을 반환
}
